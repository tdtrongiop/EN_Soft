       ...
       if (buff !=NULL){
        if (Put2Array(buff,arr,8)==0){
         ...
         for (int j=0;j<arr[4];j++){
      	    ...
      	    map[numch].det =  arr[2];
            map[numch].mod =  arr[3];
            map[numch].ch  =  arr[5]+j;
            map[numch].hit =  arr[6];
            map[numch].edge=  arr[7];
            if (arr[4]==1) sprintf(map[numch].name,"%s",detname);
...
//  runname  = "e428_run"; 
  ...
  char treename[20];
  sprintf(treename,"mapfile.map");
  if (readmapfile(treename,ENraw, numch)==-1) return -1;
//  printf("ENMain:   Khong doc dc prm file\n");
  sprintf(treename,"fiber.prm");
  fiber = new ENFiberCal(treename);
  sprintf(treename,"ppac.prm");
  ppac = new ENPPAC_Calibration(treename); 
  ppac->SetPosition(1,0,0,0,0);
...
//    Clear();
    ...
    if (f2ppacFlag==1){
        int traw[10];
        for (int i=0;i<10;i++) traw[i]= ENraw[i].val;
    ...
    if (f3ppacFlag==1){
	int traw[10];
        for (int i=0;i<10;i++) traw[i]= ENraw[f2ppacFlag*10+i].val;
	ppac->SetPosition(1,0, traw );
